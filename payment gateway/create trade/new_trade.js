...
const sell_other_form = document.querySelector("#other_method_sell"),
...
select[0].addEventListener("click", (e) => {
	document.querySelector(".buy_other_value_display").innerHTML = e.target.value;
});
...
const useData = function (data) {
	for (const key in data) {
		...
		document
			.querySelectorAll("#available_curr")
			.forEach((available) =>
...
const submitForm = function(high_limit, rate, low_limit, btn) {
	if (
		parseFloat(high_limit.value) > parseFloat(walletAvailable.innerHTML.split(" ")[1])
	) {
		e.preventDefault();
		alert("You cannot post a sell order higher than your balance 😥");
		btn.disabled = true;
		return false;
	} else if (high_limit.value == "" || low_limit.value == "" || rate.value == "") {
...
buy_wallet_submit.addEventListener("click", (e) => {
	...
	async function loadData() {
		...
		try {
			 fetch("../php/new_trade.php",{method: 'POST',
			 body:buyformData})
			.then((response) => response.json())
			.then((data) => {
				console.log(data);
				if(data.Successful){
					...
					document.querySelector('.posted_display').innerHTML = 
					`<div><span>Currency</span>   <span>${data.Successful.wallet}</span></div>
	...
	submitForm(buy_wallet_high_limit,buy_wallet_rate,buy_wallet_low_limit,buy_wallet_submit);
	loadData();
...
buy_other_submit.addEventListener("click", (e) => {
	...
	buy_otherformData.append("wallet", walletAvailable.innerHTML.split(" ")[0]);
...
sell_wallet_high_limit.addEventListener('input',(e)=>{
	document.querySelector('#sell_wallet_receive span').innerHTML = (sell_wallet_seling_rate.value *1) *(e.target.value*1);
})
sell_wallet_submit.addEventListener("click", (e) => {
	e.preventDefault()
	const sellformData = new FormData();
	sellformData.append('type','sell');
	sellformData.append("wallet", walletAvailable.innerHTML.split(" ")[0]);
	...
	submitForm(sell_wallet_high_limit,sell_wallet_seling_rate,sell_wallet_low_limit,sell_wallet_submit,e.target);
		loadData();
...
sell_other_formData.append("wallet", walletAvailable.innerHTML.split(" ")[0]);
		...
		try {
		 ...
		 .then((data) => {
			 ...
			 if(data.Successful){
				...
				document.querySelector('.sell_posted_display').innerHTML = 
				`<div><span>Currency</span>   <span>${data.Successful.wallet}</span></div>
				<div><span>Rate</span>   <span>${data.Successful.rate}</span></div>
				<div><span>Lowest</span>   <span>${data.Successful.low_limit}</span></div>
				<div><span>Highest</span>   <span>${data.Successful.high_limit}</span></div>
	...
	.then((data) => {
		...
		data.forEach((datas) => {
			...
			if (datas["google_address"]) {
				var html = ` <option value="google_pay">Google Pay</option>`;
				paymentOptions.insertAdjacentHTML("beforeend", html);
			}
			...
			if (datas["user_name"]) {
				var html = ` <option value="momo">Mobile money</option>`;
				paymentOptions.insertAdjacentHTML("beforeend", html);
			}
			if (datas["chipper"]) {
				var html = ` <option value="chipper">Chipper cash</option>`;
				paymentOptions.insertAdjacentHTML("beforeend", html);
			}
			...
			if (datas["skrill_address"]) {
				var html = ` <option value="google_pay">Skrill</option>`;
				paymentOptions.insertAdjacentHTML("beforeend", html);
			}
		});
		// console.log(data);
